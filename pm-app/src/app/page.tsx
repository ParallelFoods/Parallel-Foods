...
import { format, addDays, startOfWeek, eachDayOfInterval, isSameMonth, subDays } from 'date-fns';
...
function GanttChart() {
  ...
  const chartEnd = addDays(chartStart, 13);
        ...
        <div className="min-w-[800px]">
          ...
          <div className="relative">
            {tasks.map((task) => {
              ...
              const left = Math.max(0, (startDiff / days.length) * 100);
              const width = Math.min(100 - left, (duration / days.length) * 100);
              ...
              return (
                <div key={task.id} className="grid grid-cols-[200px_1fr] border-b border-neutral-800/30 hover:bg-neutral-800/20 transition-colors group">
                  ...
                  <div className="relative py-3 px-1 flex items-center">
                    <div className="absolute inset-0 flex">
                      {days.map(d => (
                        <div key={d.toISOString()} className="flex-1 border-r border-neutral-800/10" />
                      ))}
                    </div>
                    ...
                    {isVisible && width > 0 && (
                      <div
                        className={cn(
                          "absolute h-8 rounded-lg shadow-md flex items-center px-3 z-10 transition-all duration-300 hover:brightness-110 cursor-pointer overflow-hidden backdrop-blur-sm",
                          CATEGORY_COLORS[task.category],
                          "border"
                        )}
                        style={{ left: `${left}%`, width: `${width}%` }}
                      >
                        <span className="text-xs font-semibold truncate mix-blend-plus-lighter opacity-90">{task.taskname}</span>
                      </div>
                    )}
                  </div>
                </div>
